let verify_digital_signatures = async function verify_digital_signatures(database_connection, data, token) {
    ...
    if (data.verify_single) {
        ...
        data_verify = {token: token, party_1_digital_certificate: {digital_certificate: tutor_digital_certificate.certificate_path, digital_certificate_password: tutor_digital_certificate.certificate_password}};
    } else {
        tutor_digital_certificate = await database_connection.get_digital_certificate_details(data.tutor_email);
