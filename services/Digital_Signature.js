                    ...
                    (error, stdout, stderr) => {
                        if (error !== null) {
                            ...
                            if (error.code !== 60 && error.code !== 61) {
                                ...
                                resolve({ session_valid: true, new_token: data.token, error: true, response: { message: error_response, pdf: pdf, error_code: error.code } });
                            }
                        }

                        if (i === check_one_digital_signature_command.length - 1) {
                            resolve({ session_valid: true, new_token: data.token, error: false, response: "Digital signatures are valid." });
                        }
                    });
            }
